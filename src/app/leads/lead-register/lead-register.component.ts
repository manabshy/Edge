...
export class LeadRegisterComponent implements OnInit {
  ...
  getLeadsForCurrentUser() {
    this.storage.get("currentUser").subscribe((data: StaffMember) => {
      if (data) {
        ...
        const seeAllLeadsPermission = this.currentStaffMember.permissions.find(
          (x) => x.permissionId === 69
        );
        seeAllLeadsPermission
          ? (this.canSeeUnassignable = true)
          : (this.canSeeUnassignable = false);
        this.defaultListingType = this.canSeeUnassignable
          ? ListingType.OtherUserLeads
          : ListingType.MyLeads;
  ...
  getInfoFromLocalStorage() {
    ...
    this.storage.get("info").subscribe((data: DropdownListInfo) => {
      if (data) {
        ...
        this.infoService
          .getDropdownListInfo()
          .subscribe((res) => (this.leadTypes = res.leadTypes));
      }
    });
  ...
  getSelectedLeads() {
    this.selectedLeadsForAssignment = this.filteredLeads.filter(
      (x) => x.isChecked
    );
  }
  ...
  assignLeadsToOwner() {
    if (this.newLeadOwnerId) {
      this.leadService
        .assignLeads(this.newLeadOwnerId, this.selectedLeadsForAssignment)
        .subscribe((result) => {
          if (result) {
    ...
    if (!this.areLeadsAssignable) {
      ...
      if (this.isSubmitting) {
        ...
        localStorage.setItem(
          "listingType",
          this.leadSearchInfo.listingType.toString()
        );
      } else {
      ...
      for (const lead of this.filteredLeads) {
        newLeads.forEach((x) => {
          if (x.leadId === lead.leadId) {
            index = this.filteredLeads.findIndex(
