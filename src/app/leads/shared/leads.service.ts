...
export class LeadsService {
  ...
  public previousLeadQueryParam = new BehaviorSubject<any | null>(null);
  ...
  updateLead(lead: Lead): Observable<any> {
    ...
    return this.http.put<any>(url, lead).pipe(
      map((response) => response.result),
      tap((data) =>
        console.log("Updated Lead details here...", JSON.stringify(data))
      )
    );
  }
  ...
  getLeads(
    ...
    const dateTo = leadSearchInfo?.dateTo
      ? format(leadSearchInfo?.dateTo, "yyyy-MM-dd")
      : "";
    const dateFrom = leadSearchInfo?.dateFrom
      ? format(leadSearchInfo?.dateFrom, "yyyy-MM-dd")
      : "";
    ...
    const options = new HttpParams({
      ...
      fromObject: {
        ownerId:
          leadSearchInfo.ownerId != null
            ? leadSearchInfo.ownerId.toString()
            : "",
        personId:
          leadSearchInfo.personId != null
            ? leadSearchInfo.personId.toString()
            : "",
        leadTypeIds:
          leadSearchInfo.leadTypeIds != null
            ? leadSearchInfo?.leadTypeIds?.toString()
            : "",
        officeIds:
          leadSearchInfo.officeIds != null
            ? leadSearchInfo.officeIds?.toString()
            : "",
        dateFrom: leadSearchInfo.dateFrom != null ? dateFrom.toString() : "",
        dateTo: leadSearchInfo.dateTo != null ? dateTo.toString() : "",
        listingType:
          leadSearchInfo.listingType != null
            ? leadSearchInfo.listingType.toString()
            : "",
        includeClosedLeads:
          leadSearchInfo.includeClosedLeads != null
            ? String(leadSearchInfo.includeClosedLeads)
            : "",
        ...
        searchTerm:
          leadSearchInfo.leadSearchTerm != null
            ? leadSearchInfo.leadSearchTerm
            : "",
        allowPaging: "true",
      },
    });
  ...
  getLeadIds(
    ...
    const dateTo = leadSearchInfo?.dateTo
      ? format(parseISO(leadSearchInfo?.dateTo?.toString()), "yyyy-MM-dd")
      : "";
    const dateFrom = leadSearchInfo?.dateFrom
      ? format(parseISO(leadSearchInfo?.dateFrom?.toString()), "yyyy-MM-dd")
      : "";
    ...
    const options = new HttpParams({
      ...
      fromObject: {
        ...
        //   ? (String)(leadSearchInfo.includeUnassignedLeadsOnly) : '',
        startLeadId:
          leadSearchInfo.startLeadId != null
            ? leadSearchInfo.startLeadId.toString()
            : "",
        searchTerm:
          leadSearchInfo.leadSearchTerm != null
            ? leadSearchInfo.leadSearchTerm
  ...
  getLeadSuggestions(searchTerm): Observable<any> {
    return this.http
      .get<any>(
        `${AppConstants.baseLeadsUrl}/suggestions?SearchTerm=${searchTerm}`,
        {
