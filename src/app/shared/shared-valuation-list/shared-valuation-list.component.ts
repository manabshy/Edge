...
export class SharedValuationListComponent implements OnChanges {
  ...
  private getValuations() {
    if (this.personId) {
      this.valuations$ = this.peopleService
        .getValuations(this.personId, this.hidePrevious)
        .pipe(
          tap((vals) => {
    ...
    if (this.propertyId) {
      this.valuations$ = this.propertyService
        .getValuations(this.propertyId, this.hidePrevious)
        .pipe(
          tap((vals) => {
            this.sharedService.setValuationStatusLabel(vals);
          })
