...
export class ScoreBadgeComponent implements OnInit {
  ...
  calculateScore() {
    ...
    if (this.person) {
      ...
      if (this.person.phoneNumbers && this.person.phoneNumbers.length && this.person.phoneNumbers[0].number) {
        percent += percentRange
      } else {
        message += ', Phone number'
      }

      if (this.person.emailAddresses && this.person.emailAddresses.length && this.person.emailAddresses[0].email) {
        percent += percentRange
      } else {
