...
export class NotesComponent implements OnInit, OnChanges, OnDestroy {
  ...
  init() {
    this.notes =
      this.personNotes || this.contactGroupNotes || this.propertyNotes;
    this.setJobTypeName(this.notes);
    console.log(this.notes, "notes all");
    this.page = this.pageNumber;
    if (this.noteData) {
      this.noteData.people !== undefined
        ? (this.contactPeople = this.noteData.people)
        : (this.contactPeople = []);
      this.noteData.group
        ? (this.groupAddressee = this.noteData.group.addressee)
        : (this.groupAddressee = []);
    }
    if (this.propertyNotes) {
      this.showCreateNoteButton = false;
      this.propertyNotes.filter((x) => x.text)
        ? (this.isPropertyNote = true)
        : (this.isPropertyNote = false);
    }
    if (this.personNotes) {
      this.personNotes.filter((x) => x.text)
        ? (this.isPersonNote = true)
        : (this.isPersonNote = false);
    }
  }
