import {
  CompanyComplianceChecksSavePayload,
  ContactComplianceChecksSavePayload,
  ComplianceDocTypes,
  DOCUMENT_TYPE
} from '../compliance-checks.interfaces'
...
export const findAndRemoveDoc = (documents, ev) => {
  switch (ev.documentType) {
    ...
    case DOCUMENT_TYPE.PROOF_OF_ADDRESS:
      documents.proofOfAddressDoc.files = documents.proofOfAddressDoc.files.filter((doc) =>
        removeDocumentFilter(doc, ev.id)
      )
      break
