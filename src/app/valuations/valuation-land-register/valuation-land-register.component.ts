...
export class ValuationsLandRegisterComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  controlFiles() {
    if (!(this.property.deedLandRegFiles && this.property.deedLandRegFiles.length > 0)) {
      this.showFileUploadForDeedError = this.controlValidation ? true : false
      return false
    } else this.showFileUploadForDeedError = false
    if (!(this.property.leaseLandRegFiles && this.property.leaseLandRegFiles.length > 0) && this.showLeaseExpiryDate) {
    ...
    if (
      ...
      !(this.property.nameChangeRegFiles && this.property.nameChangeRegFiles.length > 0)
    ) {
