...
export class ContactGroupsService {
  ...
  private contactGroupAutocompleteSubject = new Subject<
    ContactGroupAutoCompleteResult[] | null
  >();
  ...
  contactGroupAutocomplete$ =
    this.contactGroupAutocompleteSubject.asObservable();
  ...
  getAutocompleteContactGroups(
    ...
    return this.http
      .get<ContactGroupAutoCompleteData>(url, { params: options })
      .pipe(
  ...
  getPersonInfo(personId: number): Observable<PersonSummaryFigures> {
    ...
    return this.http
      .get<PersonSummaryFiguresData>(url)
      .pipe(map((response) => response.result));
  }

  getPotentialDuplicatePeople(
    person: BasicPerson
  ): Observable<PotentialDuplicateResult> {
    const options = new HttpParams()
    ...
    return this.http
      .get<PeopleAutoCompleteData2>(url, { params: options })
      .pipe(
        map((response) => response.result),
  ...
  addPerson(person: Person): Observable<Person | any> {
    ...
    return this.http.post<PersonContactData>(url, person).pipe(
      map((response) => response.result),
      tap((data) =>
        console.log("added person details here...", JSON.stringify(data))
      )
    );
  }
  ...
  updatePerson(person: Person): Observable<any> {
    ...
    return this.http.put(url, person).pipe(
      ...
      tap((data) =>
        console.log("updated person details here...", JSON.stringify(data))
      )
    );
  }
  createPersonReferral(
    person: Person,
    referralCompanyId: number
  ): Observable<any> {
    const url = encodeURI(
      `${AppConstants.basePersonUrl}/${person?.personId}/referralCompany/${referralCompanyId}`
    );
    return this.http.put(url, person).pipe(
      map((response) => response),
  ...
  getAutocompleteCompany(
    ...
    pageSize?: number,
    page?: number
  ): Observable<CompanyAutoCompleteResult[]> {
    if (!page || +page === 0) {
      page = 1;
    }
    if (pageSize == null) {
    ...
    return this.http
      ...
      .pipe(
        map((response) => response.result),
        tap((data) =>
          console.log("company list here here...", JSON.stringify(data))
        )
      );
  }
  ...
  getCompany(
    ...
    return this.http.get<CompanyData>(url, { params: options }).pipe(
      map((response) => response.result),
      tap((data) =>
        console.log("company details here...", JSON.stringify(data))
      )
    );
  }
  ...
  addCompanyContactGroup(contactGroup: ContactGroup): Observable<any> {
    ...
    return this.http.post(url, contactGroup).pipe(
      ...
      tap((data) =>
        console.log(
          "added company contact details here...",
          JSON.stringify(data)
        )
      )
    );
  ...
  addPersonNote(personNote: ContactNote): Observable<ContactNote | any> {
    ...
    return this.http.post<ContactNoteData>(url, personNote).pipe(
      map((response) => response.result),
      tap((data) =>
        console.log("added  person note here...", JSON.stringify(data))
      )
    );
  }
  ...
  updatePersonNote(personNote: ContactNote): Observable<ContactNote | any> {
    ...
    return this.http.put<ContactNoteData>(url, personNote).pipe(
      map((response) => response.result),
      tap((data) =>
        console.log("updated note  person note here...", JSON.stringify(data))
      )
    );
  }

  updateContactGroupNote(
    contactGroupNote: ContactNote
  ): Observable<ContactNote | any> {
